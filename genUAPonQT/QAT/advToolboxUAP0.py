...
advMode = 'tr'
if advMode=='tr':
    x_test_uap0,x_uap_noise = uap_attack.generate(x=x_train[:N], y=y_train[:N])
elif advMode=='tst':    
    x_test_uap0,x_uap_noise = uap_attack.generate(x=x_test[:N], y=y_test[:N])
...
if advMode=='tr':
    accuracy = np.sum(np.argmax(predictions, axis=1) == np.argmax(y_train[:N], axis=1)) / N
elif advMode=='tst': 
...
#%%
#x_uap_pert = x_test_uap0-x_test[:N]
#x_uap_pertunit = x_uap_pert[0]
x_uap_pertunit = x_uap_noise
np.save('uap'+str(N)+advMode+'_'+str(epsDeg)+'.npy',x_uap_pertunit) 
