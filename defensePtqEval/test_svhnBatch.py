...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 24 20:49:20 2022
...
print('Eval Adv Res20 SVHN Model.')
...
def loadModels(path,qtModelList):
    ...
    #models.append(modelFP32)
    #modelname = 'Nat'
    modelFP32 = torch.load( './results_c10tosvhn_fulladv/Carmon2019Unlabeled_C10ftSVNH.pth' )
    modelname = 'Adv'
...
#epsList = [20,23,29,36]
epsList = [4,8,12,16,20,23,26,29,32,36]
N_eps = len(epsList)+1
...
#uap_attack = TranferUniversalPerturbation(estimatorSet=classifierSet, attacker='deepfool',eps=advEps, max_iter=10)
    if phase=='adv':
        ...
        np.save('./UAPs/ptq_svhn/uap'+str(N)+advMode+'_'+str(epsDeg)+'.npy',x_uap_noise) 
    else:
        print('Eval adv on N='+str(N)+' tst data samples of advEps='+str(epsDeg)+'/255')
        pertPath = './UAPs/ptq_svhn/'+modelType+'/'+modelMode+'/'
...
datname = './UAPs/ptq_svhn/uap'+advMode+'Results.npy'
...
datPlt = accResults.T
titlestr = 'SVHN PTQ Classification under Trainingset UAP'

plt.figure()
plt.title(titlestr)
epsList = np.array([0,4,8,12,16,20,23,26,29,32,36])
epsLen = len(epsList)
...
filestr = 'accSVHN_PTQ_white_tr.png'
