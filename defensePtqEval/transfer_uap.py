...
class TransferUniversalPerturbation(EvasionAttack):
    """
    ...
    def __init__(
        self,
        estimatorSet: list = [],#"CLASSIFIER_TYPE",
        ...
        """
        #super().__init__(estimatorSet=estimatorSet)
        self.estimatorSet=estimatorSet
    ...
    def generate(self, x: np.ndarray, y: Optional[np.ndarray] = None, **kwargs) -> np.ndarray:
        ...
        numPredictor = len(self.estimatorSet)
        print('Number of Predictors Attacked: ',numPredictor)
        
        estimator0 = self.estimatorSet[0]
        y = check_and_transform_label_format(y, estimator0.nb_classes)

        if y is None:
            # Use model predictions as true labels
            logger.info("Using model predictions as true labels.")
            y = get_labels_np_array(estimator0.predict(x, batch_size=self.batch_size))
        ...
        fooling_rate = 0.0
        nb_instances = len(x)
        sample_size = nb_instances
        #sample_size = int( nb_instances/estimator0.nb_classes*2 )

        # Instantiate the middle attacker
        attackerList = []
        for estmodel in self.estimatorSet:
            #attacker = self._get_attack(estmodel,self.attacker, self.attacker_params)
            #attacker = DeepFool(classifier=estmodel,epsilon=self.eps, max_iter=self.max_iter)
            attacker = FastGradientMethod(estimator=estmodel,eps=self.eps)
            attackerList.append(attacker)
        # Generate the adversarial examples
        nb_iter = 0
        pbar = tqdm(self.max_iter, desc="Universal perturbation", disable=not self.verbose)

        #maxRndIter = self.max_iter*numPredictor
        maxRndIter = self.max_iter
        print('Total number of attack Iter:',maxRndIter)
        while fooling_rate < 1.0 - self.delta and nb_iter < maxRndIter:
            print('nb_iter:',nb_iter)
            # Go through all the examples randomly
            rnd_idx = random.sample(range(nb_instances), sample_size)
            
            
            idxPredictor = np.random.randint(numPredictor)
            estimator = self.estimatorSet[idxPredictor]
            ...
            for j, ex in enumerate(x[rnd_idx]):
                ...
                current_label = np.argmax(estimator.predict(x_i + noise)[0])
                original_label = y_index[rnd_idx][j]

                if current_label == original_label:
                    # Compute adversarial perturbation
                    attacker = attackerList[idxPredictor]
                    adv_xi = attacker.generate(x_i + noise, y=y[rnd_idx][[j]])
                    new_label = np.argmax(estimator.predict(adv_xi)[0])
            ...
            if estimator.clip_values is not None:
                clip_min, clip_max = estimator.clip_values
                x_adv = np.clip(x_adv, clip_min, clip_max)

            # Compute the error rate
            y_adv = np.argmax(estimator.predict(x_adv, batch_size=1), axis=1)
    ...
    def _get_attack(estmodel, self, a_name: str, params: Optional[Dict[str, Any]] = None) -> EvasionAttack:
        """
        ...
        try:
            ...
            a_instance = attack_class(estmodel)  # type: ignore
